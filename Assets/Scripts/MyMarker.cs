...
public class MyMarker : MonoBehaviour
{
    ...
    [Range(0, 0.1f)] public float LineWidth = 0.02f;

    public Transform RaycastStart;
    public LayerMask DrawingLayers;

    [Range(0, 0.3f)] public float RaycastLength = 0.2f;


    [Range(0, 0.1f)] public float MinDrawDistance = 0.02f;
    [Range(0, 0.01f)] public float ReuseTolerance = 0.001f;
    ...
    private void Update()
        {
            if (drawRoutine != null)// isHolding Pen in hand
            {
        ...
        {
            foreach (Transform childLine in root)
            {
                Destroy(childLine.gameObject);
            }
        }
